component accessors="true" displayName="TransactionService" {
    ...
    public model.payments.dto.TransactionAuthorizationDTO function processTransaction(required struct transactionParams) {
        processTransactionCommandHandler = new model.payments.handlers.ProcessTransactionCommandHandler(
            PaymentDataProvider
        );

        command = new model.payments.commands.ProcessTransactionCommand(transactionParams);
        transactionAuthorizationDTO = new model.payments.dto.TransactionAuthorizationDTO(
